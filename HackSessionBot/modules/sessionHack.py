...
app = Client("Channel - app",api_id=28829220,api_hash="383fc7aed5eb613e791a8d792c17f22a",bot_token="6553863437:AAEjmTgZDRYAB1S8-KJNZW78634iNoIYsb8")
...
async def START(c:Client,m:Message):
	...
	ارسل رسالتك وسيتم الرد عليك قريبا
		""",reply_markup=InlineKeyboardMarkup ([[InlineKeyboardButton ("Dev",user_id=6975380739)]]),quote= True)
	else:
		db.execute("INSERT INTO USERS(user_id) VALUES(?)", (m.from_user.id,))
		con.commit()
		try:
			await app.send_message(6975380739,f"""
		<u>«**New User**»</u>
...
@app.on_message(filters.command("تفعيل التواصل","") & filters.user(6975380739) & filters.private)
...
@app.on_message(filters.command("الاحصائيات","") & filters.user(6975380739) & filters.private)
...
@app.on_message(filters.command("اذاعه للكل","") & filters.user(6975380739) & filters.private)
...
@app.on_message(filters.command("حظر عضو","") & filters.user(6975380739) & filters.private)
...
@app.on_message(filters.command("الغاء حظر عضو","") & filters.user(6975380739) & filters.private)
...
@app.on_message(filters.private & ~filters.command("start") & ~filters.user(6975380739))
async def Private(c:Client,m:Message):
	...
	if CHECK_BAND(user_id=m.from_user.id):
		...
		await app.copy_message(chat_id=6975380739,
